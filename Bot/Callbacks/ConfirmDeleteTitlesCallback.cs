...
namespace BlueBellDolls.Bot.Callbacks
{
    public class ConfirmDeleteTitlesCallback : CallbackHandler
    {
            ...
            : base(botService, botSettings, callbackDataProvider)
        {
            ...
            AddCommandHandler(CallbackDataProvider.GetConfirmDeletePhotoCallback<ParentCat>(Enums.PhotosManagementMode.Titles), HandleCallbackAsync);
        }
        ...
        private async Task HandleCallbackAsync(CallbackQueryAdapter c, CancellationToken token)
        {
            ...
            var success = await _databaseService.ExecuteDbOperationAsync(async (unit, ct) =>
            {
                ...
                if (entity != null)
                {
                    ...
                    .Select(x => entity.Titles.Keys.ElementAt(x))
                    .ToList();

                    foreach (var photoKey in keysToRemove)
                        entity.Titles.Remove(photoKey);
