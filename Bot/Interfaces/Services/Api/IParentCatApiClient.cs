...
namespace BlueBellDolls.Bot.Interfaces.Services.Api
{
    public interface IParentCatApiClient : IDisplayableEntityApiClient<ParentCatDetailDto>
    {
        ...
        Task<EntityFilesUploadResult<ParentCatDetailDto>?> UploadGenTestsAsync(int id, IEnumerable<(Stream fileStream, string fileName, string fileId)> photos, CancellationToken token = default);
        Task<EntityFilesUploadResult<ParentCatDetailDto>?> UploadTitlesAsync(int id, IEnumerable<(Stream fileStream, string fileName, string fileId)> photos, CancellationToken token = default);
